...
async def show_stats(call: CallbackQuery):
    ...
    text = (
        ...
        f"├ {tg(T_PROFILE, '💰')} <b>Балансы пользователей:</b> ${total_balance:.2f}\n"
        ...
        f"└ {tg(T_CAT, '⭐')} <b>Новых за 7 дней:</b> {new_7d}"
    )
    await show_menu(call, text, back_to_admin_kb())
